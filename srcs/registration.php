...
<center>
<?php
	$user_name = preg_replace('/\s+/', '', $_POST['username']);
	$passwd = preg_replace('/\s+/', '', $_POST['pwd1']);
	$emailAdd = preg_replace('/\s+/', '', $_POST['email']);
	$f_name = preg_replace('/\s+/', '', $_POST['fname']);
	$l_name = preg_replace('/\s+/', '', $_POST['lname']);
	$ddp = "media/dps/default.jpg";
	try
	{
		include_once("config/database.php");
		if (!empty($user_name) && !empty($passwd) && !empty($_POST['pwd2']) && !empty($emailAdd) && !empty($f_name) && !empty($l_name))
		{
			if (isset($user_name) && isset($passwd) && isset($_POST['pwd2']) && isset($emailAdd) && isset($f_name) && isset($l_name))
			{
				if(ctype_alpha($f_name) && ctype_alpha($l_name))
				{
